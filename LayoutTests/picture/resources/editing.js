...
function execInsertPictureCommand(imgSrc) {
    document.execCommand("InsertPicture", false, imgSrc);
    debugForDumpAsText("execInsertPictureCommand");
}
function insertPictureCommand(imgSrc) {
    if (commandDelay > 0) {
        queueCommand(execInsertPictureCommand.bind(execInsertPictureCommand, imgSrc), commandCount * commandDelay);
        commandCount++;
    }
    else {
        execInsertPictureCommand(imgSrc);
    }
}
